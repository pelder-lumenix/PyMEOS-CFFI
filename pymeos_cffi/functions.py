...
def geo_as_geojson(gs: 'const GSERIALIZED *', option: int, precision: int, srs: "Optional[str]") -> str:
    gs_converted = _ffi.cast('const GSERIALIZED *', gs)
...
def geoset_transform_pipeline(s: 'const Set *', pipelinestr: str, srid: int, is_forward: bool) -> 'Set *':
    s_converted = _ffi.cast('const Set *', s)
...
def point_transform_pipeline(gs: 'const GSERIALIZED *', pipelinestr: str, srid: int, is_forward: bool) -> 'GSERIALIZED *':
    gs_converted = _ffi.cast('const GSERIALIZED *', gs)
    pipelinestr_converted = pipelinestr.encode('utf-8')
    srid_converted = _ffi.cast('int32', srid)
...
def stbox_transform_pipeline(box: 'const STBox *', pipelinestr: str, srid: int, is_forward: bool) -> 'STBox *':
    box_converted = _ffi.cast('const STBox *', box)
    pipelinestr_converted = pipelinestr.encode('utf-8')
    srid_converted = _ffi.cast('int32', srid)
...
def tpoint_at_geom_time(temp: 'const Temporal *', gs: 'const GSERIALIZED *', zspan: "Optional['const Span *']", period: "Optional['const Span *']") -> 'Temporal *':
    temp_converted = _ffi.cast('const Temporal *', temp)
    gs_converted = _ffi.cast('const GSERIALIZED *', gs)
    zspan_converted = _ffi.cast('const Span *', zspan) if zspan is not None else _ffi.NULL
    period_converted = _ffi.cast('const Span *', period) if period is not None else _ffi.NULL
...
def tpoint_minus_geom_time(temp: 'const Temporal *', gs: 'const GSERIALIZED *', zspan: "Optional['const Span *']", period: "Optional['const Span *']") -> 'Temporal *':
    temp_converted = _ffi.cast('const Temporal *', temp)
    gs_converted = _ffi.cast('const GSERIALIZED *', gs)
    zspan_converted = _ffi.cast('const Span *', zspan) if zspan is not None else _ffi.NULL
    period_converted = _ffi.cast('const Span *', period) if period is not None else _ffi.NULL
...
def stbox_space_tile(point: 'const GSERIALIZED *', xsize: float, ysize: float, zsize: float, sorigin: 'const GSERIALIZED *') -> 'STBox *':
    point_converted = _ffi.cast('const GSERIALIZED *', point)
    sorigin_converted = _ffi.cast('const GSERIALIZED *', sorigin)
    result = _lib.stbox_space_tile(point_converted, xsize, ysize, zsize, sorigin_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def stbox_space_time_tile(point: 'const GSERIALIZED *', t: int, xsize: float, ysize: float, zsize: float, duration: 'const Interval *', sorigin: 'const GSERIALIZED *', torigin: int) -> 'STBox *':
    point_converted = _ffi.cast('const GSERIALIZED *', point)
    t_converted = _ffi.cast('TimestampTz', t)
    duration_converted = _ffi.cast('const Interval *', duration)
    sorigin_converted = _ffi.cast('const GSERIALIZED *', sorigin)
    torigin_converted = _ffi.cast('TimestampTz', torigin)
    result = _lib.stbox_space_time_tile(point_converted, t_converted, xsize, ysize, zsize, duration_converted, sorigin_converted, torigin_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tfloat_value_split(temp: 'const Temporal *', size: float, origin: float) -> "Tuple['Temporal **', 'double *', 'int']":
    temp_converted = _ffi.cast('const Temporal *', temp)
...
def tfloat_value_time_split(temp: 'const Temporal *', size: float, duration: 'const Interval *', vorigin: float, torigin: int) -> "Tuple['Temporal **', 'double *', 'TimestampTz *', 'int']":
    temp_converted = _ffi.cast('const Temporal *', temp)
    duration_converted = _ffi.cast('const Interval *', duration)
    torigin_converted = _ffi.cast('TimestampTz', torigin)
    value_buckets = _ffi.new('double **')
...
def tfloatbox_value_time_tile(value: float, t: int, vsize: float, duration: 'const Interval *', vorigin: float, torigin: int) -> 'TBox *':
    t_converted = _ffi.cast('TimestampTz', t)
    duration_converted = _ffi.cast('const Interval *', duration)
    torigin_converted = _ffi.cast('TimestampTz', torigin)
    result = _lib.tfloatbox_value_time_tile(value, t_converted, vsize, duration_converted, vorigin, torigin_converted)
    _check_error()
    return result if result != _ffi.NULL else None


def tfloatbox_value_time_tiles(box: 'const TBox *', xsize: float, duration: 'const Interval *', xorigin: 'Optional[float]', torigin: "Optional[int]") -> "Tuple['TBox *', 'int']":
    box_converted = _ffi.cast('const TBox *', box)
    duration_converted = _ffi.cast('const Interval *', duration)
    ...
    count = _ffi.new('int *')
    result = _lib.tfloatbox_value_time_tiles(box_converted, xsize, duration_converted, xorigin_converted, torigin_converted, count)
    _check_error()
    return result if result != _ffi.NULL else None, count[0]
...
def tint_value_split(temp: 'const Temporal *', size: int, origin: int) -> "Tuple['Temporal **', 'int *', 'int']":
    temp_converted = _ffi.cast('const Temporal *', temp)
...
def tintbox_value_time_tile(value: int, t: int, vsize: int, duration: 'const Interval *', vorigin: int, torigin: int) -> 'TBox *':
    t_converted = _ffi.cast('TimestampTz', t)
    duration_converted = _ffi.cast('const Interval *', duration)
    torigin_converted = _ffi.cast('TimestampTz', torigin)
    result = _lib.tintbox_value_time_tile(value, t_converted, vsize, duration_converted, vorigin, torigin_converted)
    _check_error()
    return result if result != _ffi.NULL else None
...
def tpoint_space_split(temp: 'const Temporal *', xsize: 'float', ysize: 'float', zsize: 'float', sorigin: 'const GSERIALIZED *', bitmatrix: bool, border_inc: bool) -> "Tuple['Temporal **', 'GSERIALIZED ***', 'int']":
    ...
    count = _ffi.new('int *')
    result = _lib.tpoint_space_split(temp_converted, xsize_converted, ysize_converted, zsize_converted, sorigin_converted, bitmatrix, border_inc, space_buckets, count)
    _check_error()
...
def tpoint_space_time_split(temp: 'const Temporal *', xsize: 'float', ysize: 'float', zsize: 'float', duration: 'const Interval *', sorigin: 'const GSERIALIZED *', torigin: int, bitmatrix: bool, border_inc: bool) -> "Tuple['Temporal **', 'GSERIALIZED ***', 'TimestampTz *', 'int']":
    ...
    time_buckets = _ffi.new('TimestampTz **')
    count = _ffi.new('int *')
    result = _lib.tpoint_space_time_split(temp_converted, xsize_converted, ysize_converted, zsize_converted, duration_converted, sorigin_converted, torigin_converted, bitmatrix, border_inc, space_buckets, time_buckets, count)
    _check_error()
...
def tbox_value_time_tile(value: 'Datum', t: int, vsize: 'Datum', duration: 'const Interval *', vorigin: 'Datum', torigin: int, basetype: 'meosType') -> 'TBox *':
    value_converted = _ffi.cast('Datum', value)
    t_converted = _ffi.cast('TimestampTz', t)
    ...
    torigin_converted = _ffi.cast('TimestampTz', torigin)
    basetype_converted = _ffi.cast('meosType', basetype)
    result = _lib.tbox_value_time_tile(value_converted, t_converted, vsize_converted, duration_converted, vorigin_converted, torigin_converted, basetype_converted)
    _check_error()
    return result if result != _ffi.NULL else None
